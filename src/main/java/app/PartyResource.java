 ...
 *
 * <p>This API allows clients to create, retrieve, list, and delete party entities. Each party is
 * identified by a unique ID and supports flexible metadata.
 *
...
public class PartyResource extends TmfApiBase<Party, PartyRepo> {
  ...
  @Operation(
      ...
      description =
          "Adds a new party to the database using the provided JSON payload. Returns the created party's details.")
  @Parameters({
    @Parameter(
        name = "data",
        description = "The JSON payload containing the party details",
        required = true)
  })
  @RequestBody(
      content =
        ...
        .recoverWithItem(
            throwable ->
                Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity(
                        Map.of(
                            "error", "Failed to create party", "details", throwable.getMessage()))
                    .build());
  }
  ...
  public Uni<Response> getPartyById(@PathParam("id") String id) {
    return this.getEntityById(UUID.fromString(id))
        ...
        .onItem()
        .ifNull()
        .failWith(() -> new NotFoundException("Party not found"))
        .map(party -> Response.ok(party).build())
        .onFailure(NotFoundException.class)
        .recoverWithItem(
            throwable ->
                Response.status(Response.Status.NOT_FOUND).entity(throwable.getMessage()).build())
        .onFailure()
        .recoverWithItem(
            throwable ->
                Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity(
                        Map.of(
                            "error", "Failed to retrieve party", "details", throwable.getMessage()))
                    .build());
  }
  ...
  @APIResponses({
    @APIResponse(
        responseCode = "200",
        description = "List of parties returned successfully",
  ...
  public Uni<Response> deletePartyById(@PathParam("id") String id) {
    return super.delete(UUID.fromString(id))
        .onItem()
        .transform(deleted -> Response.noContent().build())
        .onFailure(NotFoundException.class)
        .recoverWithItem(
            throwable ->
                Response.status(Response.Status.NOT_FOUND).entity(throwable.getMessage()).build())
        ...
        .recoverWithItem(
            throwable ->
                Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity(
                        Map.of(
                            "error", "Failed to delete party", "details", throwable.getMessage()))
                    .build());
  }
   ...
   */
  @DELETE
  @Operation(summary = "Delete all parties", description = "Removes all parties from the database.")
  @APIResponses({
        ...
        .recoverWithItem(
            throwable ->
                Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity(
                        Map.of(
                            "error", "Failed to delete parties", "details", throwable.getMessage()))
                    .build());
  }
            ...
            throwable ->
                Response.status(Response.Status.NOT_FOUND)
                    .entity(
                        Map.of(
                            "details",
                            "Party not found for ID : %s".formatted(id),
                            "error",
        ...
        .recoverWithItem(
            throwable ->
                Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity(
                        Map.of(
                            "error", "Failed to update party", "details", throwable.getMessage()))
                    .build());
  }
}
